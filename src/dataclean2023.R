...
library(lattice)
source("C:/Users/alsipl/Desktop/ZuurSpatialGLM/AllData/HighstatLibV13.R")
...
# GSL outline
sf<-st_read("~/MSc/data/GSLsf/NWT_Detailed_LakesRivers_GSL.shp")
...
mz<-st_read("~/MSc/data/GSLsf/GSL_managementzones.shp")
# Reading layer ... using driver `ESRI Shapefile'
...
rv<-st_read("~/MSc/data/GSLsf/Med_Canada_Rivers.shp")
# Reading layer ... using driver `ESRI Shapefile'
...
# Boat catch + lim ----------
boat_raw <- read.csv("~/MSc/data/boatcatch.csv")
...
land_raw <- read.csv("~/MSc/data/proccatch.csv")
...
bathyFIGS <- read.csv("~/MSc/data/allcoord.csv") #xlsx file is GSL2016allgridcoord
...
ZONE_IE<-readOGR(dsn='C:/Users/alsipl/Desktop/JackDistFromShore', layer='ZONE IE')
...
crs(ZONE_IE)<-'+proj=longlat +datum=WGS84 +no_defs'
ZONE_IE.UTM=spTransform(ZONE_IE, geo_proj)#ensure projection is correct 

crs(ZONE_IW)<-'+proj=longlat +datum=WGS84 +no_defs'
ZONE_IW.UTM=spTransform(ZONE_IW, geo_proj)
...
FISH.SHP.UTM=raster::union(ZONE_IE.UTM, ZONE_IW.UTM)
# plot(FISH.SHP.UTM)
